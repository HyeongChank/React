...
const Taccident = () => {
    const apiData = {
      ...
      //오브젝트는 키, 값으로 이루어짐
      console.log("오브젝트 확인 data", data);
      console.log("오브젝트 확인 apidate", apiData);
      //배열로 만들어서 순회 및 찾기 가능하게 만듬
      let objk = Object.keys(apiData);
      let objV = Object.values(apiData);
      console.log("Object key 순회", objk);
      console.log("Object values 순회", objV);
      
      //배열의 map(키와 값으로 분리), filter
      let newv;
      ...
      console.log("data추출", data);
      data = data.map((k) => apiData[k]);

      //오브젝트 키로 접근
      data = apiData.data;
      console.log("오브젝트 키로 접근",data);
      data = apiData['data'];
      console.log("오브젝트 키로 접근",data);

      //대분류 추출
      let c1 = data.map((item) => 
      item['사고유형_대분류']
      );
      // console.log(c1);
        //중복값 제거 set
      ...
      // item.사고유형_대분류+','+item.사고유형_중분류).splice(',');
